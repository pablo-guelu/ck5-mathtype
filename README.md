...
This repo shows a simple installation demo of MathType NODE package for CKEditor 5.
...
Node or Node.js is a runtime environment that can run JavaScript code. Before Node, JavaScript was intended to be used only in the context of web browsers. 
But later some dude saw its potential to also use it as server-side language. Similar to PHP, Java, Python, etc.

Remember that for running such server-side apps you need an environment in which to run them. For example, for running a PHP-based server app you would use [XAMPP](https://www.apachefriends.org/) or a similar environment to respond to requests and execute PHP code. In the same way, you'd use Node to execute Javascript, but not in the browser, but in a server. [More on Node.js here](https://developer.mozilla.org/en-US/docs/Learn/Server-side/Express_Nodejs/Introduction#introducing_node).

Node packages are reusable code libraries made by different people that **can be added to node projects**. To manage those packages and install them easily we use a tool called **NPM (node package manager).**. NPM is also a CLI tool that lets you run commands to execute your node application (but we won't need those commands for this demo, only the package installation). [More on NPM here](https://docs.npmjs.com/about-npm).
...
The tool used to bundle the NODE module of MathType and CKEditor (and others) in this demo is **webpack** (the most widely used probably). **A great explanation of why to use a bundler and what is webpack** [here](https://webpack.js.org/concepts/why-webpack/).
